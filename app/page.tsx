...
export default function HomePage() {
    ...
    try {
      ...
      if (vendorsData.length === 0) {
        const defaultVendors = [
          { name: 'Dangote', type: 'Cement Supplier', contact: '080-DANGOTE' },
          { name: 'BUA', type: 'Cement Supplier', contact: '080-BUA' },
          { name: 'Julius Berger', type: 'Contractor', contact: '080-JB' },
          { name: 'Emos', type: 'Materials', contact: '080-EMOS' },
          { name: 'Schneider', type: 'Electrical', contact: '080-SCHNEIDER' }
        ]
      ...
      if (categoriesData.length === 0) {
        const defaultCategories = [
          { name: 'Cement', color: '#gray' },
          { name: 'Blocks', color: '#red' },
          { name: 'Sand', color: '#yellow' },
          { name: 'Labour', color: '#blue' },
          { name: 'Transport', color: '#green' },
          { name: 'Wood', color: '#brown' },
          { name: 'Roofing', color: '#purple' },
          { name: 'Electrical', color: '#orange' },
          { name: 'Plumbing', color: '#cyan' },
          { name: 'Paint', color: '#pink' },
          { name: 'Tiles', color: '#indigo' },
          { name: 'Doors/Windows', color: '#teal' }
        ]
  ...
  const getFilteredExpenses = () => {
    ...
    switch(timeFilter) {
      ...
      case 'quarter':
        ...
        filtered = filtered.filter(e => new Date(e.createdAt) >= quarterAgo)
        break
    }
  ...
  // Chart Data
  const getChartData = () => {
    const projectNames = projects.map(p => p.name.split(' ')[0])
    const projectBudgets = projects.map(p => p.budget)
    const projectSpent = projects.map(p => {
      const expenses = getProjectExpenses(p.name)
      ...
      <div className="bg-gray-900 border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex space-x-8">
            {['dashboard', 'expenses', 'projects', 'vendors', 'reports'].map(tab => (
              <button
                key={tab}
            ...
            {showAnalytics && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 lg:col-span-2">
                  <h3 className="text-xl font-semibold mb-4">30-Day Expense Trend</h3>
                    ...
                    <div key={project.id} className="bg-gray-900 rounded-xl p-6 border border-gray-800 hover:border-gray-700 transition-all">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="flex space-x-2">
                          <button
                            onClick={() => setShowEditProject(project)}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between text-gray-400">
                          ...
                          <span className="text-white">{project.contractor || 'N/A'}</span>
                        </div>
                        <div className="flex justify-between text-gray-400">
                          <span>Status:</span>
        ...
        {activeTab === 'reports' && (
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
                <h3 className="text-xl font-semibold mb-4">Budget vs Spent Analysis</h3>
                <Bar data={chartData.bar} options={{
                  responsive: true,
                  plugins: {
                    legend: { position: 'top' }
              ...
              <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 lg:col-span-2">
                <h3 className="text-xl font-semibold mb-4">30-Day Trend Analysis</h3>
                <Line data={chartData.line} options={{
                  responsive: true,
                  plugins: {
                    legend: { position: 'top' }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              />
              <select
                value={showEditExpense.project}
                onChange={(e) => setShowEditExpense({...showEditExpense, project: e.target.value})}
                className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-blue-500"
              >
                <option value="">Select Project</option>
                {projects.map(p => (
              ...
              <input
                type="text"
                placeholder="Vendor"
                value={showEditExpense.vendor}
                onChange={(e) => setShowEditExpense({...showEditExpense, vendor: e.target.value})}
                className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-blue-500"
              />
              <select
            ...
            <div className="flex justify-end space-x-4 mt-6">
              ...
              <button
                onClick={() => handleUpdateExpense(showEditExpense.id, showEditExpense)}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors"
              >
                Update Expense
              </button>
          ...
          <div className="bg-gray-900 rounded-xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <input
                type="text"
                placeholder="Project Name"
                value={showEditProject.name}
                onChange={(e) => setShowEditProject({...showEditProject, name: e.target.value})}
                className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-blue-500"
              />
              <input
                type="number"
                placeholder="Budget"
                value={showEditProject.budget}
                onChange={(e) => setShowEditProject({...showEditProject, budget: parseFloat(e.target.value)})}
                className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-blue-500"
              />
              <input
                type="text"
                placeholder="Location"
                value={showEditProject.location}
                onChange={(e) => setShowEditProject({...showEditProject, location: e.target.value})}
                className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-blue-500"
              />
              <select
                value={showEditProject.status}
                onChange={(e) => setShowEditProject({...showEditProject, status: e.target.value})}
                className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-blue-500"
              >
                <option value="active">Active</option>
                <option value="on-hold">On Hold</option>
            ...
            <div className="flex justify-end space-x-4 mt-6">
              ...
              <button
                onClick={() => handleUpdateProject(showEditProject.id, showEditProject)}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors"
              >
                Update Project
              </button>
