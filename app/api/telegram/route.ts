  ...
  try {
    ...
    if (cmd === 'export') {
      await sendMessage(chatId, 
        ...
        'https://property-tracker-1nyeo6jpx-ibrahim-abubakars-projects.vercel.app\n\n' +
        'Click "Export Excel" or "Export PDF" button to download all data.'
      )
      return NextResponse.json({ ok: true })
    }
    ...
    if (amount > 0) {
      ...
      // VENDOR DETECTION - Clean, single logic
      let vendor = 'Unknown'
      
      // Check for known company vendors
      ...
      projects.forEach(doc => {
        const data = doc.data()
        const projectKey = data.name.toLowerCase().split(' ')[0]
