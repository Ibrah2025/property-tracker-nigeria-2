...
export async function POST(req) {
  try {
    ...
    if (cmd.startsWith('search')) {
      ...
      if (!term) {
        await sendMessage(chatId, '⚠️ Search what? Example: search Dangote')
        return NextResponse.json({ ok: true })
      }
    ...
    if (numMatch) {
      ...
      if (!session.cache[num]) {
        await sendMessage(chatId, '⚠️ Invalid #. Type LIST first')
        return NextResponse.json({ ok: true })
      }
      ...
      if (action === 'delete') {
        ...
        if (amount) {
          ...
          await sendMessage(chatId, `✏️ #${num + 1} updated to ${formatMoney(amount)}`)
        } else {
          await sendMessage(chatId, '⚠️ Use: #3 delete or #3 500k')
        }
      }
      return NextResponse.json({ ok: true })
    }
    ...
    if (cmd === 'cancel' || cmd === 'undo') {
      if (session.lastId) {
        ...
        await sendMessage(chatId, '⚠️ No recent expense')
      }
      return NextResponse.json({ ok: true })
    }
    ...
    if (cmd.startsWith('edit ')) {
      if (!session.lastId) {
        await sendMessage(chatId, '⚠️ No recent expense to edit')
        return NextResponse.json({ ok: true })
      }
      ...
      if (amount) {
        ...
        await sendMessage(chatId, `✏️ Updated to ${formatMoney(amount)}`)
      } else {
        await sendMessage(chatId, '⚠️ Invalid amount')
      }
      return NextResponse.json({ ok: true })
    }
